 ...
 * 幂等服务接口。
 *
 * <p>幂等的目标不是“拦截重复点击”，而是保证同一个业务请求被重复发送时，
 * 不会重复产生业务副作用。</p>
 *
...
public interface IdempotencyService {
     ...
     *
     * @param bizType 业务类型，例如 CREATE_ORDER，用于隔离不同业务的幂等 key
     * @param key 调用方传入的幂等键，通常来自 Idempotency-Key 请求头
     * @param requestHash 当前请求内容摘要，用于判断同一个 key 是否被不同请求体复用
     * @return 幂等检查结果；firstRequest=true 表示当前请求可以执行业务
     */
     ...
     * 标记业务处理成功。
     *
     * <p>业务成功后保存响应体，后续相同 key + 相同请求体的重复请求可以直接返回第一次结果。</p>
     */
